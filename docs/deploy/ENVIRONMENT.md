...
default**. Nothing else reads `process.env` — all env access goes through
`src/config.ts`.
...
shell/process — never written to disk. Prefer the env var over the flag
...
`.env.example` should carry this block:
